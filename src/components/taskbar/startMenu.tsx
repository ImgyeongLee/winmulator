...
export default function StartMenu() {
    ...
    return (
        <div
            className={
                'ml-[0.5px] main-container absolute z-99 bottom-[33px] flex flex-col justify-start  ' +
            ...
            <div className={'header rounded-[8px] flex items-center gap-2 xp-start-menu-header-gradient px-1 py-2'}>
                <div className={'border border-[2px] border-neutral-100/70 rounded-[5px]'}>
                    <Image width={35} height={20} src={'/xp/startMenu/userImage1.webp'} alt={'user account image'} />
                </div>
            ...
            <div
                className={
                    'body-content flex-grow grid grid-cols-2 border border-xp-start-menu-right-border' +
                    ' bg-white mx-[3px]'
                }>
                <div className={'left-side flex flex-col ml-1 mt-1'}>
                        ...
                        {startMenuLeftItems.map(([path, label, sublabel, width], index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={cn(
                                        'cursor-pointer flex items-center gap-2 mb-2 py-1 text-black hover:bg-xp-taskbar-selected pl-1 mr-1 group',
                                        {
                                            'mb-1 py-0': sublabel,
                                        }
                                    )}>
                                    ...
                                    <div className={'flex flex-col group-hover:text-white'}>
                                        ...
                                        <h3 className={'text-[0.7rem] text-gray-900/60 mr-auto group-hover:text-white'}>
                                            {sublabel}
                                        </h3>
                                    </div>
                                </div>
                    ...
                    <div
                        className={
                            'all-prog text-black font-bold text-[0.8rem] flex items-center justify-end items-center ' +
                    ...
                    {startMenuRightItems.map(([path, label, subMenu], index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={cn(
                                    'flex items-center gap-2 pl-1 mb-2 py-1 text-black cursor-pointer hover:bg-xp-taskbar-selected hover:text-white',
                                    {}
                                )}>
                                <Image
                                    width={20}
                                ...
                                />
                                <h3
                                    className={cn('text-[0.8rem] mr-auto', {
                                        'font-bold': index < 5,
                                    })}>
                                    {label}
                                </h3>
                                {subMenu && <IoTriangleSharp className={'mr-2 rotate-90 text-[0.45rem]'} />}
                            </div>
                        );
                    })}
                </div>
            </div>
            <div className={'footer flex flex-row items-center justify-end h-[40px]'}>
                <button
                    className={
                        'flex mr-[18px] items-center text-[0.9rem] p-1 cursor-pointer hover:bg-xp-taskbar-selected/50'
                    }>
                    <Image
                        width={20}
                        height={20}
                        src={'/xp/startMenu/log_off.webp'}
                        alt={'log off button'}
                        className={'mr-1'}
                    />
                    <h3>Log off</h3>
                ...
                <button
                    className={
                        'flex mr-4 items-center text-[0.9rem] p-1 cursor-pointer hover:bg-xp-taskbar-selected/50'
                    }>
                    <Image
                        width={20}
                        height={20}
                        src={'/xp/startMenu/turn_off.webp'}
                        alt={'log off button'}
                        className={'mr-1'}
                    />
                    <h3>Shut Down</h3>
